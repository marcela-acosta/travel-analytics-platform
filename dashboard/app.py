...
tab1, tab2 = st.tabs(["📊 Dashboard", "💬 Chat with your Pipeline"])
...
with tab2:
    ...
    st.markdown("### Ask anything about your pipeline")
    st.caption("Powered by Gemini 2.0 Flash · dbt gold layer")
    ...
    if prompt := st.chat_input("e.g. How many deals are overdue? Which agent has the best win rate?"):
        ...
        with st.chat_message("assistant"):
            with st.spinner("Analyzing your pipeline data..."):
                try:
                    reply = _agent_chat(prompt, st.session_state.chat_history, raw)
